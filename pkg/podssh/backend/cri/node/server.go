// Package node runs the privileged node-local half of the Pod SSH CRI data plane.
package node
...
// Options configures the node-local CRI data plane.
type Options struct {
...
// Run serves the node-local data plane until ctx is cancelled.
...
func defaultOptions(options Options) Options {
	if options.ListenAddress == "" {
		options.ListenAddress = ":" + strconv.Itoa(cri.DefaultStreamPort)
	}
	if options.ManagementAddress == "" {
		options.ManagementAddress = ":" + strconv.Itoa(cri.DefaultMetricsPort)
	}
	...
	if options.ExpectedClientName == "" {
		options.ExpectedClientName = cri.DefaultClientName
	}
...
func newServer(ctx context.Context, options Options, runtime runtimeClient, runtimeEndpoint string) *server {
	info := version.Get()
...
func (s *server) streamHandler() http.Handler {
	mux := http.NewServeMux()
	mux.HandleFunc("POST /"+cri.APIVersion+"/exec/{namespace}/{pod}/{uid}/{container}", s.observe("exec", s.handleExec))
	mux.HandleFunc("POST /"+cri.APIVersion+"/helper/{namespace}/{pod}/{uid}/{container}/{capability}", s.observe("helper", s.handleHelper))
	mux.HandleFunc("POST /"+cri.APIVersion+"/portforward/{namespace}/{pod}/{uid}", s.observe("portforward", s.handlePortForward))
...
func (s *server) handleExec(w http.ResponseWriter, r *http.Request) {
	...
	response, err := s.runtime.Exec(r.Context(), &runtimeapi.ExecRequest{
		...
		Cmd:         query[cri.QueryCommand],
		Stdin:       queryBool(query, cri.QueryStdin),
		Stdout:      queryBool(query, cri.QueryStdout),
		Stderr:      queryBool(query, cri.QueryStderr),
		Tty:         queryBool(query, cri.QueryTTY),
	})
	if err != nil {
...
func (s *server) handleHelper(w http.ResponseWriter, r *http.Request) {
	...
	query := r.URL.Query()
	command := append([]string{helperPath}, query[cri.QueryArgument]...)
	response, err := s.runtime.Exec(r.Context(), &runtimeapi.ExecRequest{
		ContainerId: target.ContainerID, Cmd: command,
		Stdin: queryBool(query, cri.QueryStdin), Stdout: queryBool(query, cri.QueryStdout), Stderr: queryBool(query, cri.QueryStderr),
	})
	if err != nil {
...
func (s *server) handlePortForward(w http.ResponseWriter, r *http.Request) {
	...
	port, err := strconv.ParseInt(r.URL.Query().
		Get(cri.QueryPort), 10, 32)
...
func loadServerTLS(options Options) (*tls.Config, *dynamicTLSRuntime, error) {

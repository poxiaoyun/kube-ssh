...
func TestExecHandlerResolvesCRIAndProxies(t *testing.T) {
	...
	server := &server{
		options: Options{ExpectedClientName: cri.DefaultClientName},
		runtime: runtime,
	...
	request.TLS = &tls.ConnectionState{PeerCertificates: []*x509.Certificate{{Subject: pkix.Name{CommonName: cri.DefaultClientName}}}}
...
func TestStreamHandlerRejectsWrongClientIdentity(t *testing.T) {
	server := &server{options: Options{ExpectedClientName: cri.DefaultClientName}}
...
func TestDefaultOptionsIncludesCommonRuntimeEndpoints(t *testing.T) {

...
func TestLoadServerTLSReloadsServingCertificateAndClientCA(t *testing.T) {
	...
	tlsConfig, runtime, err := loadServerTLS(Options{
		TLSCAFile:   caFile,
...
func waitForTLSReload(t *testing.T, config *tls.Config, oldCertificate []byte, oldCAs *x509.CertPool) *tls.Config {
	t.Helper()
	deadline := time.Now().
		Add(5 * time.Second)
	for time.Now().
		...
		clientCAsChanged := !current.ClientCAs.Equal(oldCAs)
		if certificateChanged && clientCAsChanged {
			return current
		}
...
func writeTLSFiles(t *testing.T, caFile, certFile, keyFile string, generation int64) {
	...
	caTemplate := &x509.Certificate{
		...
		Subject: pkix.Name{CommonName: "test-ca-" + big.NewInt(generation).
			String()},

...
func TestExecHelperRequiresHelperPath(t *testing.T) {
	transport := New(nil, nil, Options{})
	_, err := transport.ExecHelper(context.Background(), backend.HelperExecRequest{
		StreamRequest: backend.StreamRequest{Target: podtarget.New("default", "app", "app")},
		Capability:    helperpkg.CapabilitySFTP,
		Command:       []string{helperpkg.CommandSFTP},
	})
	if !apierrors.IsServiceUnavailable(err) {
		t.Fatalf("ExecHelper() error = %v, want ServiceUnavailable", err)
	}
}

func TestExecHelperPreservesCopyCause(t *testing.T) {
	for _, cause := range []error{apierrors.NewNotFound(corev1.Resource("pods"), "app"), context.Canceled} {
		t.Run(cause.Error(), func(t *testing.T) {
			transport := New(nil, nil, Options{HelperPath: writeTempHelper(t, "helper-binary")})
			transport.execOverride = func(context.Context, backend.ExecRequest) (int, error) {
				return 1, cause
			}
			_, err := transport.ExecHelper(context.Background(), backend.HelperExecRequest{
				StreamRequest: backend.StreamRequest{Target: podtarget.New("default", "app", "app")},
			...
			if !errors.Is(err, cause) {
				t.Fatalf("ExecHelper() error = %v, want cause %v", err, cause)
			}
			if apierrors.IsNotFound(cause) && !apierrors.IsNotFound(err) {
				t.Fatalf("ExecHelper() error = %v, want NotFound", err)
			}
		})
	}
}
...
func TestAcquireHelperRecordsActiveUntilFirstRelease(t *testing.T) {
	...
	acquired, err := b.acquireHelper(context.Background(), podtarget.New("default", "nginx", "app"), helperpkg.CapabilitySFTP)
	if err != nil {
...
func TestHelperCacheKeyUsesVersionIdentity(t *testing.T) {
	tgt := podtarget.Bind(&corev1.Pod{ObjectMeta: metav1.ObjectMeta{Namespace: "default", Name: "nginx", UID: "uid-1"}}, "app")
	manifest := validHelperManifest()
	key := helperCacheKey(tgt, manifest)
	replacement := podtarget.Bind(&corev1.Pod{ObjectMeta: metav1.ObjectMeta{Namespace: "default", Name: "nginx", UID: "uid-2"}}, "app")
	if got := helperCacheKey(replacement, manifest); got == key {
		t.Fatal("replacement Pod reused helper cache key")
	}
	...
	b := &Transport{
		execOverride: func(_ context.Context, req backend.ExecRequest) (int, error) {
			...
			case helperpkg.CommandVersion:
				...
				versionCount++
				mu.Unlock()
				_ = json.NewEncoder(req.Stdout).
					Encode(validHelperManifest())
			default:
				mu.Lock()
				copyCount++
...
func TestAcquireHelperDifferentTargetsCopySeparately(t *testing.T) {
	...
	targets := []*target.Target{
		podtarget.New("default", "nginx-a", "app"),
		podtarget.New("default", "nginx-b", "app"),
	}
	...
	for _, tgt := range targets {
		if _, err := b.acquireHelper(context.Background(), tgt, helperpkg.CapabilitySFTP); err != nil {
			t.Fatalf("acquireHelper(%s) error = %v", tgt.String(), err)
		}
	}
	if copyCount != 2 {
...
func (s *helperExecScript) backend() *Transport {
	return &Transport{
		execOverride: func(_ context.Context, req backend.ExecRequest) (int, error) {
			step := len(s.commands)

...
func newCopyHelperAcquirer(backend *Transport, opts copyHelperAcquirerOptions) *copyHelperAcquirer {
	if opts.RemoteDir == "" {
		opts.RemoteDir = "/tmp"
	}
...
func (a *copyHelperAcquirer) Acquire(ctx context.Context, tgt *target.Target, capability string) (helperLease, error) {
	if a.localPath == "" {
		return nil, apierrors.NewServiceUnavailable(fmt.Sprintf("helper path is required for %s", capability))
	}
...
func (a *copyHelperAcquirer) copyHelperBinary(ctx context.Context, tgt *target.Target, data []byte, manifest helperpkg.Manifest) (string, error) {
	...
	for _, method := range a.copyMethods() {
		err := method.copy(ctx, tgt, remotePath, data)
		if err != nil {
			failures = append(failures, fmt.Errorf("%s: %w", method.name, err))
			continue
		}
		return remotePath, nil
	}
	return "", fmt.Errorf("inject helper: %w", errors.Join(failures...))
}
...
func (a *copyHelperAcquirer) copyHelperBinaryWithShell(ctx context.Context, tgt *target.Target, remotePath string, data []byte) error {
	...
	if exitCode != 0 {
		return apierrors.NewServiceUnavailable(fmt.Sprintf("helper copy exited with %d", exitCode))
	}
	return nil
}
...
func (a *copyHelperAcquirer) probeVersion(ctx context.Context, tgt *target.Target, remotePath string) (helperpkg.Manifest, error) {
	...
	if err != nil {
		return helperpkg.Manifest{}, fmt.Errorf("run helper version: %w", err)
	}
	if exitCode != 0 {
		return helperpkg.Manifest{}, apierrors.NewServiceUnavailable(fmt.Sprintf("helper version exited with %d: %s", exitCode, bytes.TrimSpace(stderr.Bytes())))
	}

	manifest := helperpkg.Manifest{}
	if err := json.Unmarshal(stdout.Bytes(), &manifest); err != nil {
		return helperpkg.Manifest{}, fmt.Errorf("decode helper version: %w", err)
	}
	return manifest, nil
}
...
func helperCachePrefix(tgt *target.Target) string {
	return tgt.String() + "\x00" + podtarget.BoundIdentity(tgt).UID + "\x00"
}

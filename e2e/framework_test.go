...
func TestMissingKubernetesTargetsAreRejected(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if strings.Contains(output, "is not allowed") || strings.Contains(output, "no regular container") || strings.Contains(output, "get target pod") {
...
func TestMissingHelperFailsClearly(t *testing.T) {
	...
	output := sftp.Stdout + sftp.Stderr

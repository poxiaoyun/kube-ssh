...
| Backend                  | 后端执行边界。只负责把已解析、已授权的 shell/exec/sftp/portforward 操作落到具体运行时。默认 Kubernetes backend 使用 `pods/exec`/`pods/portforward`；Node backend 直连节点并使用 CRI v1 streaming。 |
...
| 模块                        | 职责                                                                                                                                   |
| --------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| API Server + SAR            | 承载 Pod/Access/Secret watch/get 与 SubjectAccessReview；仅默认 Kubernetes backend 的数据流使用 `pods/exec`、`pods/portforward`。 |
...
高流量场景可将 backend 切换为严格的 Node 模式：Gateway 根据已解析 Pod
...
Node 模式不会在故障时回退到 apiserver streaming。apiserver 只保留控制面和
...
默认 Kubernetes backend 由网关通过 `pods/exec` 注入；Node backend 则由节点
